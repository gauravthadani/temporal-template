...
class EncryptionCodec(PayloadCodec):
    ...
    async def decode(self, payloads: Iterable[Payload]) -> List[Payload]:
        ...
        for p in payloads:
            ...
            # Decrypt and append
            decrypt: object = self.decrypt(p.data)

            ret.append(Payload(metadata={"encoding": b"unauthorized",
                                         "message": b"boo hoo buddy, no decrypt for you"
                                         }, data=bytes({})))
            # ret.append(Payload.FromString(decrypt))
        return ret
